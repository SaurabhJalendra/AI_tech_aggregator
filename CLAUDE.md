...
### ConstraintState-only (final)

Flat `constraints` in `client_context` is **rejected**. See [docs/CONSTRAINT_STATE.md](./docs/CONSTRAINT_STATE.md) and [docs/PRODUCTION_READINESS.md](./docs/PRODUCTION_READINESS.md).

### Production hardening (planner pass)

| Concern | Mechanism |
|---------|-----------|
| Prompt boundary | `validate_client_context()` + `payload_sanitizer.py` + `format_prompt_context()` |
| Deterministic ranking | `sort_scored_records()` — score, confidence, retrieval_score, slug |
| Filter exhaustion | `constraint_negotiation.py` option cards (no silent fallback) |
| Planner rollback | `PLANNER_MODE=off\|shadow\|on` in `.env` |
| Telemetry | SSE `planner_telemetry`; internal `GET /api/v1/advisor/metrics/internal` |
| Subprocess safety | `subprocess_lifecycle.terminate_subprocess()` |
